...
// ---- A change the platform declined to render ---------------------------------

/**
 * The shape `add-local-git-investigation` exists for (task 3.7): a change
 * whose files a platform enumerated in full and then would not serve the
 * content of. Two ordinary source files, one of which the source declines —
 * so a harness test can prove the declined file blocks completion while the
 * served one does not, from the same manifest.
 *
 * A diff body is written for the declined file even though nothing serves it.
 * That is the point of the fixture: the content exists and is perfectly
 * readable, and the reason it is not read is a decision the source made.
 */
export const DECLINED_CONTENT_FILE_PATH = 'src/billing/invoice.ts';
export const DECLINED_CONTENT_SERVED_PATH = 'src/billing/rounding.ts';

export const DECLINED_CONTENT_REF: ChangeRequestRef = { repoId: 'harness-declined', number: '1' };

export const DECLINED_CONTENT_DIFF: ChangeRequestDiff = {
  ref: DECLINED_CONTENT_REF,
  baseSha: 'declined-base-1',
  headSha: 'declined-head-1',
  files: [
    {
      oldPath: DECLINED_CONTENT_FILE_PATH,
      newPath: DECLINED_CONTENT_FILE_PATH,
      diff:
        '@@ -1,3 +1,4 @@\n export function invoiceTotal(lines: readonly number[]) {\n-  return lines.reduce((a, b) => a + b, 0)\n+  return Math.round(lines.reduce((a, b) => a + b, 0) * 100) / 100\n }\n',
    },
    {
      oldPath: DECLINED_CONTENT_SERVED_PATH,
      newPath: DECLINED_CONTENT_SERVED_PATH,
      diff: '@@ -1,1 +1,2 @@\n export const CENTS = 100\n+export const HALF_CENT = 0.5\n',
    },
  ],
  anchorRefs: { base_sha: 'declined-base-1', head_sha: 'declined-head-1' },
};
