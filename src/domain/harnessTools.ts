...
export interface HostToolDefinition {
  ...
  /**
   * The method the dispatcher must find defined on whatever answers this tool,
   * before dispatch, independent of the declared capability — task 9.2's
   * `capabilityUnavailable` also fires when the method is undefined.
   *
   * Which object that is depends on the tool: the five pinned operations and
   * `resolvePolicy` are answered by the member's `InvestigationSource`, the two
   * detail reads by its `Connection`. It is the union of both because one
   * generic check covers all eight read tools with no special case, and
   * `dispatchTargetFor` is the one place that decides which object the name is
   * looked up on. Undefined for the two host actions, which call neither.
   */
  readonly connectionMethod?: keyof Connection | keyof InvestigationOperations;
