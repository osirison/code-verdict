...
describe('HarnessPolicy defaults and normalization (task 2.6)', () => {
  it('matches the design defaults table when given nothing', () => {
    ...
    // Deliberately above the design table's original 64 KB, together with the read-page bounds
    // below: at 64 KB a 2,025-line lock-file diff arrived as eleven sequential model round trips,
    // which was over three minutes of a four-minute review. The three move together — a page a
    // provider may return has to be a result the budget will accept.
    expect(policy.maxToolResultBytes).toBe(256 * 1024);
    expect(policy.diffOrFileReadPageLines).toBe(20_000);
    expect(policy.diffOrFileReadPageBytes).toBe(256 * 1024);
  ...
  it('defaults scopeInvestigationToChangedFiles to false, and falls back to it on a non-boolean value', () => {
    // False since every read became local: the metered API call that justified
    // keeping a review inside its own change request does not exist any more.
    expect(DEFAULT_HARNESS_POLICY.scopeInvestigationToChangedFiles).toBe(false);
    expect(normalizeHarnessPolicy({}).scopeInvestigationToChangedFiles).toBe(false);
    expect(normalizeHarnessPolicy({ scopeInvestigationToChangedFiles: 'yes' }).scopeInvestigationToChangedFiles).toBe(false);
    // A reviewer who wants the narrow behaviour still gets exactly it.
    expect(normalizeHarnessPolicy({ scopeInvestigationToChangedFiles: true }).scopeInvestigationToChangedFiles).toBe(true);
  });
});
