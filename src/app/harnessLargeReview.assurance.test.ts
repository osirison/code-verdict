...
function largeCapabilities(): MemberCapabilities {
...
/**
 * Two pages, chosen by the cursor the caller sends rather than by how many
 * times this has been called — which is how a real manifest endpoint behaves,
 * and which matters since `add-local-git-investigation` task 9.1: a run now
 * enumerates the manifest twice, once for source selection's serviceability
 * check (design D6) and once for the attempt's own inventory, and a handler
 * counting its own calls would hand the second enumeration the tail of the
 * first and silently shrink the inventory to half the change.
 */
function manifestHandler(): (request: ChangedFileManifestRequest) => Promise<ChangedFileManifestResult> {
  return async (request) => {
    if (request.cursor === undefined) {
      return { snapshot: request.snapshot, state: 'paginated', value: [manifestEntry(HIGH_FILE), manifestEntry(LOW_FILES[0]!)], cursor: 'manifest-page-2' };
    }
...
describe('16.2: a review whose manifest and bootstrap evidence exceed a single prompt', () => {
  it('pages the manifest to exhaustion, reopens the summarized bootstrap section, reserves and uses the high-risk investigation reserve once ordinary is spent, produces exact (non-estimated) coverage, and reaches a truthful complete outcome', async () => {
    ...
    const deps: HarnessRuntimeDeps = { ...baseDeps(), runTurn, policy, harnessRunStore };
    ...
    // 1. The manifest paged to exhaustion: exactly the two pages the fake source defines, never
    // estimated or guessed at. It was four calls while source selection enumerated the manifest a
    // second time to decide whether the provider could serve the change; that check went with the
    // provider investigation path, so the run enumerates once and this is the attempt's own
    // inventory paging to exhaustion.
...
describe('16.2: budget genuinely runs out before every required file is covered — the run ends explicitly partial, never a clean result it did not earn', () => {
  it('one validated finding survives, but the review reports partial with a named coverage limitation, never complete or clean', async () => {
    ...
    const deps: HarnessRuntimeDeps = { ...baseDeps(), runTurn, policy };
    const factory = createReviewHarnessFactory(deps);
