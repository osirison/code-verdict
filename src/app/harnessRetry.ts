...
export interface RetryHooks {
  ...
  /**
   * Fires once per *inline* backoff wait this call actually takes — a delay classified short
   * enough to sit out under this same `await` (9.6), never one long enough to become a `wait`
   * outcome (that branch never sleeps at all; see `onEnterWaiting` instead). `info.delayMs` is the
   * exact wait this module already computed (`computeBackoffMs`/provider `Retry-After`) — a
   * caller wanting a running total of retry time should sum these, never recompute its own timer
   * around the call. Fires after the wait settles, whether it elapsed in full or was cut short by
   * cancellation.
   */
  readonly onRetryWait?: (info: RetryWaitInfo) => void;
}
...
export async function runWithRetry<T>(fn: () => Promise<T>, options: RunWithRetryOptions): Promise<RetryOutcome<T>> {
  ...
  for (;;) {
    ...
    try {
      ...
      options.hooks?.onRetryWait?.({ attempt: attempts, delayMs });
