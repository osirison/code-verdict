...
function timedToolCompleted(sequence: number, occurredAt: string, timing: { durationMs?: number; bytesReceived?: number; retryWaitMs?: number; retryCount?: number }, target = 'file1.ts'): ActivityEvent {
  return { ...base(sequence, occurredAt), kind: 'toolCompleted', tool: 'readDiff', target, summary: '1 unit(s) returned.', ...timing };
}
...
describe('compactActivity (11.3)', () => {
  ...
  it('sums timing/size metadata across a folded run rather than keeping only the last event\'s value — timings must survive a checkpoint round trip', () => {
    const events = [
      timedToolCompleted(1, '2026-01-01T00:00:00.000Z', { durationMs: 50, bytesReceived: 100, retryWaitMs: 10, retryCount: 1 }),
      timedToolCompleted(2, '2026-01-01T00:00:01.000Z', { durationMs: 30, bytesReceived: 200 }), // no retry on this one
      timedToolCompleted(3, '2026-01-01T00:00:02.000Z', { durationMs: 20, bytesReceived: 50, retryWaitMs: 5, retryCount: 1 }),
    ];
    const result = compactActivity(events, GENEROUS_POLICY);
    ...
    expect(folded?.kind).toBe('toolCompleted');
    if (folded?.kind === 'toolCompleted') {
      // A fold that silently kept only the last event's duration (20/50/5/1) would undercount by
      // exactly what this test pins: the other two calls' durations and bytes.
      expect(folded.durationMs).toBe(100); // 50 + 30 + 20
      expect(folded.bytesReceived).toBe(350); // 100 + 200 + 50
      expect(folded.retryWaitMs).toBe(15); // 10 + 5 (the middle call contributed nothing, not a fabricated 0 forced into the sum wrongly)
      expect(folded.retryCount).toBe(2);
    }
  });

  it('leaves timing/size fields unset when no event in the run ever carried them — never a fabricated 0 for an attempt persisted before this metadata existed', () => {
    const events = [toolCompleted(1, '2026-01-01T00:00:00.000Z'), toolCompleted(2, '2026-01-01T00:00:01.000Z')];
    const result = compactActivity(events, GENEROUS_POLICY);
    const folded = result.events[0];
    expect(folded?.kind).toBe('toolCompleted');
    if (folded?.kind === 'toolCompleted') {
      expect(folded.durationMs).toBeUndefined();
      expect(folded.bytesReceived).toBeUndefined();
      expect(folded.retryWaitMs).toBeUndefined();
