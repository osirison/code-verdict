...
import { parseSourceCitation, quotedTextAppearsIn, resolveCandidateCitations, resolveCitation } from './harnessCitations';
...
/**
 * The verbatim-quote rule, stated for a payload whose every line carries a prefix byte.
 *
 * `validateCandidate` used to ask `exactContent.includes(code)` of every kind of evidence. On a
 * patch that is wrong in both directions at once, and the strict direction destroyed real
 * findings: a model quoting two consecutive added lines as the file reads them sends them without
 * the `+`, and the payload has one on each.
 */
describe('quotedTextAppearsIn — what "verbatim in the evidence" means for a patch', () => {
  const PATCH = ['@@ -1,2 +1,4 @@', ' const a = 1;', '-const b = 2;', '+const b = 3;', '+const c = 4;', ' return a;'].join('\n');

  function patchSource(): LedgerEvidenceSource {
    const ledger = createEvidenceLedger(IDENTITY, [M1]);
    const diff = ledger.registerDiffPage('m1', { state: 'complete', snapshot: SNAP1, value: { path: 'src/auth/token.ts', patch: PATCH, positions: [] } });
    if (!diff.ok) throw new Error('setup failed');
    return diff.source;
  }

  it('accepts one line, part of one line, and consecutive lines of the same side', () => {
    const source = patchSource();
    expect(quotedTextAppearsIn(source, 'const c = 4;')).toBe(true);
    expect(quotedTextAppearsIn(source, '= 4')).toBe(true);
    expect(quotedTextAppearsIn(source, 'const b = 3;\nconst c = 4;')).toBe(true); // added, added
    expect(quotedTextAppearsIn(source, 'const a = 1;\nconst b = 2;')).toBe(true); // context, deleted — the old side
    expect(quotedTextAppearsIn(source, 'const c = 4;\nreturn a;')).toBe(true); // added, context
  });

  it('refuses a run that only exists in the payload once the prefixes are counted as code', () => {
    const source = patchSource();
    // Each of these is a substring of the payload, which is exactly why the old rule passed them.
    for (const quote of ['const b = 2;\n+const b = 3;', '@@ -1,2 +1,4 @@', '+const c = 4;']) {
      expect(source.exactContent.includes(quote)).toBe(true);
      expect(quotedTextAppearsIn(source, quote)).toBe(false);
    }
    // Deleted then added: never two consecutive lines of any file.
    expect(quotedTextAppearsIn(source, 'const b = 2;\nconst b = 3;')).toBe(false);
    expect(quotedTextAppearsIn(source, 'eval(userInput)')).toBe(false);
  });

  it('leaves every other kind of evidence on the plain substring test its own bytes deserve', () => {
    const { file } = setup();
    expect(quotedTextAppearsIn(file, 'xxx')).toBe(true);
    expect(quotedTextAppearsIn(file, 'yyy')).toBe(false);
  });
});
