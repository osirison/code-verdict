...
import { PRE_LOCAL_GIT_LINEAGE_KEY, preLocalGitLineageRecord } from './migrationFixtures';
...
// ---- The investigation source and what the base revision means (tasks 9.5-9.8) --------

/**
 * `add-local-git-investigation` tasks 9.5, 9.6, 9.7 and 9.8, spec
 * `agentic-review-harness`: "A resumed attempt keeps the investigation source
 * and base-revision meaning it started with."
 *
 * The order these are compared in is the substance, not a detail. A stored
 * `baseSha` is forty hex characters and says nothing about whether it is a merge
 * base or the tip of a target branch, so comparing the two strings first answers
 * a question nobody asked when the meaning behind them changed — and answers it
 * with wording ("the base revision changed from X to Y") that tells a reviewer
 * their target branch moved when nothing of the sort happened.
 */
describe('a new attempt keeps the source and the base-revision meaning it started with (tasks 9.5-9.8)', () => {
  const FORBIDDEN_WORDING = [/reconnect/i, /reattach/i, /\bresum(e|ed|ing)\b/i, /\bcontinu(e|ed|ing|ation)\b/i];

  function withMember(snapshot: ReviewRunSnapshot, member: Partial<ReviewRunSnapshot['members'][number]>): ReviewRunSnapshot {
    return { ...snapshot, members: [{ ...snapshot.members[0]!, ...member }] };
  }

  const LOCAL_SOURCE = { kind: 'localGit' as const, contractVersion: '1', capabilitySignature: 'sig-local' };
  const PROVIDER_SOURCE = { kind: 'provider' as const, contractVersion: '1', capabilitySignature: 'sig-provider' };

  it('a different source kind is its own incompatibility, with its own reason', () => {
    const stored = withMember(testSnapshot(), { investigationSource: PROVIDER_SOURCE, baseRevisionKind: 'mergeBase' });
    const candidate = withMember(testSnapshot(), { investigationSource: LOCAL_SOURCE, baseRevisionKind: 'mergeBase' });

    const reasons = checkSnapshotCompatibility(stored, candidate);

    expect(reasons.map((reason) => reason.code)).toEqual(['investigationSource']);
    expect(reasons[0]!.message).toContain('the platform this change request lives on');
    expect(reasons[0]!.message).toContain('a local copy of the repository');
    for (const pattern of FORBIDDEN_WORDING) expect(reasons[0]!.message).not.toMatch(pattern);
  });

  it('the same source kind with a different capability signature or contract version is not an incompatibility of its own', () => {
    const stored = withMember(testSnapshot(), { investigationSource: { kind: 'provider', contractVersion: '1', capabilitySignature: 'sig-a' } });
    const candidate = withMember(testSnapshot(), { investigationSource: { kind: 'provider', contractVersion: '2', capabilitySignature: 'sig-b' } });

    // Task 9.5 names the kind and only the kind. What the attempt could actually
    // do is already compared as `providerCapabilitySignature`, which is a member
    // field of its own and is unchanged here.
    expect(checkSnapshotCompatibility(stored, candidate)).toEqual([]);
  });

  it('a base revision recorded under the earlier meaning is incompatible, and never reported as a target branch that moved', () => {
    const stored = withMember(testSnapshot(), { baseRevisionKind: 'targetBranchTip', baseSha: 'tip-of-target-branch' });
    const candidate = withMember(testSnapshot(), { baseRevisionKind: 'mergeBase', baseSha: 'the-merge-base' });

    const reasons = checkSnapshotCompatibility(stored, candidate);

    expect(reasons.map((reason) => reason.code)).toEqual(['baseRevisionMeaning']);
    expect(reasons[0]!.message).toContain('the tip of the target branch');
    expect(reasons[0]!.message).toContain('the commit this change branched from');
    // The existing wording, which is true only when the meaning stayed the same,
    // must not also fire here — it is a claim about a different fact.
    expect(reasons[0]!.message).not.toContain('changed from');
    expect(reasons.some((reason) => reason.code === 'headRevision')).toBe(false);
  });

  it('the ordinary case still reports a moved base in the ordinary words', () => {
    const stored = withMember(testSnapshot(), { baseRevisionKind: 'mergeBase', baseSha: 'base1' });
    const candidate = withMember(testSnapshot(), { baseRevisionKind: 'mergeBase', baseSha: 'base2' });

    const reasons = checkSnapshotCompatibility(stored, candidate);

    expect(reasons.map((reason) => reason.code)).toEqual(['headRevision']);
    expect(reasons[0]!.message).toContain('base revision changed from base1 to base2');
  });

  /**
   * Task 9.6, and the one case in this area that could reasonably have gone
   * either way. Design D4 settles it: the pair of commits the stored evidence
   * was computed over is the pair this attempt would compute over — byte for
   * byte the same two commits — so the meaning alone settles nothing. Relabeling
   * is what would be dishonest here, and there is nothing to relabel: the new
   * attempt records the current meaning because `buildReviewRunSnapshot` writes
   * `mergeBase` into every snapshot it builds.
   */
  it('the meaning changed but the commit did not: compatible, and the new attempt records the current meaning', async () => {
    const stored = withMember(testSnapshot(), { baseRevisionKind: 'targetBranchTip', baseSha: 'same-commit' });
    const candidate = withMember(testSnapshot(), { baseRevisionKind: 'mergeBase', baseSha: 'same-commit' });
    ...
    const store = createHarnessRunStore(backing, { now: () => 0 });
    await store.writeSnapshot(stored);
    const decision = decideResume({ storedSnapshot: stored, checkpoint: testCheckpoint(stored), candidateSnapshot: candidate });
    expect(decision.kind).toBe('compatible');

    // What a compatible decision leads to: the candidate is written, and what it
    // records is the meaning in force now.
    await store.writeSnapshot({ ...candidate, attempt: 2 });
    expect(store.readSnapshot(LINEAGE_ID as never, 2 as never)?.members[0]?.baseRevisionKind).toBe('mergeBase');
  });

  /**
   * Task 9.7, over the recorded pre-change lineage (`migrationFixtures.ts`) read
   * through the store's own path rather than a hand-built object — the question
   * is what a record already on somebody's disk compares as, and only a record
   * written before the fields existed can answer it.
   */
  describe('a stored record that predates both fields (task 9.7)', () => {
    function storedPreLocalGitSnapshot(): ReviewRunSnapshot {
      const map = new Map<string, unknown>();
      const backing = {
        get: <T,>(key: string) => (map.has(key) ? (JSON.parse(JSON.stringify(map.get(key))) as T) : undefined),
        update: async (key: string, value: unknown) => {
      ...
      map.set(PRE_LOCAL_GIT_LINEAGE_KEY, preLocalGitLineageRecord());
      const store = createHarnessRunStore(backing, { now: () => 0 });
      return store.readSnapshot('lineage-pre-local-git' as never, 1 as never)!;
    }

    it('reads as a target-branch tip served by the provider, and compares as those against a candidate that says so explicitly', () => {
      const stored = storedPreLocalGitSnapshot();
      expect('baseRevisionKind' in stored.members[0]!).toBe(false);
      expect('investigationSource' in stored.members[0]!).toBe(false);

      const candidate: ReviewRunSnapshot = {
        ...stored,
        members: [{ ...stored.members[0]!, baseRevisionKind: 'targetBranchTip', investigationSource: PROVIDER_SOURCE }],
      };

      // Absence is a fact, so the two records describe the same thing and
      // nothing about them is incompatible.
      expect(checkSnapshotCompatibility(stored, candidate)).toEqual([]);
    });

    it('is incompatible with a new attempt that would read the same commits from a local store', () => {
      const stored = storedPreLocalGitSnapshot();
      const candidate: ReviewRunSnapshot = {
        ...stored,
        members: [{ ...stored.members[0]!, baseRevisionKind: 'targetBranchTip', investigationSource: LOCAL_SOURCE }],
      };
    ...
    it('is incompatible for the base revision’s meaning when the new attempt pins the merge base to a different commit', () => {
      const stored = storedPreLocalGitSnapshot();
      const candidate: ReviewRunSnapshot = {
        ...stored,
        members: [{ ...stored.members[0]!, baseRevisionKind: 'mergeBase', baseSha: 'aaaa1111bbbb2222cccc3333dddd4444eeee5555', investigationSource: PROVIDER_SOURCE }],
      };
  ...
  /**
   * Task 9.8. Two sources do not produce interchangeable evidence — the same
   * file's diff has different bytes and therefore a different digest, and the
   * same change has a different manifest — so nothing from one may be replayed
   * into an attempt served by the other. The proof is structural rather than a
   * count of what survived: an incompatible decision has no payload to carry
   * anything in, and a fresh restart calls nothing in this module at all.
   */
  it('carries no coverage, evidence or inspection state across a source change', () => {
    const stored = withMember(testSnapshot(), { investigationSource: PROVIDER_SOURCE });
    const candidate = withMember(testSnapshot(), { investigationSource: LOCAL_SOURCE });
    const checkpoint = testCheckpoint(stored, {
      coverage: ZERO_COVERAGE,
      candidates: [acceptedCandidate('c1', fakeSource('src-1', 'exact diff bytes from the provider'))],
      evidence: [toRetainedEvidenceRecord(fakeSource('src-1', 'exact diff bytes from the provider'), { includeExactContent: true })],
    });

    const decision = decideResume({ storedSnapshot: stored, checkpoint, candidateSnapshot: candidate });

    expect(decision.kind).toBe('incompatible');
    if (decision.kind !== 'incompatible') return;
    expect(decision.reasons.map((reason) => reason.code)).toEqual(['investigationSource']);
    // There is no shape here that could hand a caller the prior attempt's
    // coverage, evidence or candidates: the incompatible arm carries reasons and
    // nothing else.
    expect(Object.keys(decision)).toEqual(['kind', 'reasons']);
  });
});
