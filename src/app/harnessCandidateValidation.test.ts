...
/**
 * The shape a real run actually validates against.
 *
 * Every fixture above this block hands `registerDiffPage` a list of `positions`; the only
 * production investigation source (`../localgit/localGitSource.ts`) returns `positions: []` on
 * every page, and the patch it returns is whole `git diff` output — file header, `@@` lines, and a
 * `+`, `-` or space in front of every line of code. Against that shape, and only against that
 * shape, this module rejected every finding a live 245-file review submitted: first at
 * `primary:pathMismatch`, because a page with no positions had no citable location at all, and
 * then — once locations were derived from the patch — at `codeNotInEvidence`, because a quote of
 * two consecutive lines as the *file* reads them is not a substring of patch bytes that carry a
 * prefix on each line. Both are fixed; this block is what keeps them fixed.
 */
describe('validateCandidate against a production-shaped diff page', () => {
  const REAL_PATCH = [
    'diff --git a/src/auth/token.ts b/src/auth/token.ts',
    'index 3015688..780ed7a 100644',
    '--- a/src/auth/token.ts',
    '+++ b/src/auth/token.ts',
    '@@ -8,4 +8,6 @@ import { read } from \'./io\';',
    ...
    '+  return { token, expiry };',
    ' }',
    '@@ -40,2 +42,3 @@ export function clear() {',
    '   cache.clear();',
    '+  audit.log("cleared");',
    ' }',
  ].join('\n');
  ...
  function productionFixture(): Fixture {
    const ledger = createEvidenceLedger(IDENTITY, [M1, M2]);
    const diff = ledger.registerDiffPage('m1', {
      state: 'complete',
      snapshot: SNAP1,
      value: { path: 'src/auth/token.ts', patch: REAL_PATCH, positions: [] },
    });
    const file = ledger.registerFileRange('m2', fileRange());
    const att = ledger.registerAttachment('m1', attachment(), sha256Hex(attachment().content));
    if (!diff.ok || !file.ok || !att.ok) throw new Error('setup failed');
    return {
      ...
      file: file.source,
      attachment: att.source,
      context: { ledger, criteria: DEFAULT_CRITERIA, changedPathsByMember: new Map([['m1', new Set(['src/auth/token.ts'])]]), now: NOW },
    };
  }

  function submit(fixture: Fixture, overrides: Record<string, unknown> = {}) {
    return validateCandidate(
      candidate(fixture, {
        line: 11,
        code: 'if (!expiry) throw new Error("no expiry");',
        citations: { primary: cite(fixture.diff, 'src/auth/token.ts', 11) },
        ...overrides,
      }),
      fixture.context,
    );
  ...
  it('accepts a well-formed finding about a page whose source named no positions', () => {
    const outcome = submit(productionFixture());
    expect(outcome).toMatchObject({ state: 'accepted' });
    if (outcome.state !== 'accepted') return;
    expect(outcome.finding.routing).toBe('inline');
    expect(outcome.finding.evidence.primary).toMatchObject({ path: 'src/auth/token.ts', range: { startLine: 11, endLine: 11 } });
  });

  it('accepts a multi-line quote of consecutive lines on one side of one hunk, and rejects one that crosses sides', () => {
    const fixture = productionFixture();
    expect(submit(fixture, {
      line: 10,
      endLine: 11,
      code: '  const expiry = token.expires_at;\n  if (!expiry) throw new Error("no expiry");',
      citations: { primary: cite(fixture.diff, 'src/auth/token.ts', 10, 11) },
    })).toMatchObject({ state: 'accepted' });
    // A deleted line and an added line are consecutive in the payload's bytes but were never
    // consecutive in any file, on either side.
    expect(submit(fixture, { code: '  return token;\n  const expiry = token.expires_at;' }))
      .toMatchObject({ state: 'rejected', reasons: [{ code: 'codeNotInEvidence' }] });
  });

  it('rejects a quote that is only a substring of the raw patch — prefixes, headers and hunk boundaries are not code', () => {
    const fixture = productionFixture();
    // Each of these appears verbatim in `source.exactContent`, which is exactly why the old
    // whole-payload substring test passed them.
    for (const code of [
      'expires_at;\n+  if (!expiry)',
      '+++ b/src/auth/token.ts',
      '@@ -40,2 +42,3 @@',
      ...
      expect(fixture.diff.exactContent.includes(code)).toBe(true);
      expect(submit(fixture, { code })).toMatchObject({ state: 'rejected', reasons: [{ code: 'codeNotInEvidence' }] });
    }
  });

  it('still rejects a quote that is nowhere in the evidence', () => {
    expect(submit(productionFixture(), { code: 'eval(userInput)' })).toMatchObject({ state: 'rejected', reasons: [{ code: 'codeNotInEvidence' }] });
  });

  it('still rejects a citation to lines the page never carried, and a finding pointing outside its own citation', () => {
    const fixture = productionFixture();
    // Line 30 sits in the gap between the two hunks: in the file, never in the page.
    expect(submit(fixture, { line: 30, code: undefined, citations: { primary: cite(fixture.diff, 'src/auth/token.ts', 30) } }))
      .toMatchObject({ state: 'rejected', reasons: [{ code: 'primary:rangeOutsideEvidence' }] });
    // Cited 11, pointed at 20: inside the page, outside the span the citation named.
    expect(submit(fixture, { line: 20, code: undefined }))
      .toMatchObject({ state: 'rejected', reasons: [{ code: 'locationOutsideEvidence' }] });
  });
});
