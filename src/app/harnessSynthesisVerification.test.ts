...
/** The directive as the stage would build it: choose the excerpt, then format it. Fails loudly if the excerpt is unshowable. */
function directiveFor(finding: ValidatedFinding, source: LedgerEvidenceSource): string {
  const excerpt = selectEvidenceExcerpt(finding, source);
  if (excerpt.kind === 'unavailable') throw new Error(`expected a presentable excerpt, got unavailable: ${excerpt.reason}`);
  return buildContradictionDirective(finding, excerpt);
}
...
/**
 * The failure these cover: the directive used to carry `exactContent.slice(0, 4_000)`,
 * so a finding citing anything past that prefix was judged against bytes that did not
 * contain it — and a `contradicted: true` answer to that question silently removed a
 * validated finding. Every assertion here is about *which lines the model was handed*.
 */
describe('evidence excerpt anchoring', () => {
  /** ~68 characters a line, so 120 lines is comfortably past the 4,000-character budget. */
  function bigPatch(lineCount: number): string {
    const lines = [`@@ -1,${lineCount} +1,${lineCount} @@`];
    for (let i = 1; i <= lineCount; i += 1) lines.push(`+const value${i} = 'marker-${i}-${'x'.repeat(40)}';`);
    return lines.join('\n');
  }

  function recordingSeam(contradicted = false): HarnessModelSeam & { calls: number; directives: string[] } {
    const seam = {
      modelId: 'test-model',
      calls: 0,
      ...
      async askModel({ repairInstruction }: { repairInstruction: string | undefined }) {
        seam.calls += 1;
        seam.directives.push(repairInstruction ?? '');
        return verdictJson(extractCandidateId(repairInstruction), contradicted, contradicted ? 'The cited lines do the opposite of the claim.' : undefined);
      },
    };
    return seam;
  }
  ...
  it('a finding citing the END of an over-budget page is shown the cited lines, not the first 4,000 characters', async () => {
    const ledger = makeLedger();
    const patch = bigPatch(120);
    expect(patch.length).toBeGreaterThan(4_000);
    const source = registerDiff(ledger, 'src/big.ts', patch, 120);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-end', title: 'Unchecked value near the end' }, source, 'src/big.ts', 118, 119));
    ...
    // The bytes under judgement are in front of the model...
    expect(directive).toContain("const value118 = 'marker-118-");
    expect(directive).toContain("const value119 = 'marker-119-");
    // ...and the start of the page, which the old prefix showed instead, is not.
    expect(directive).not.toContain("const value1 = 'marker-1-");
    expect(result.complete).toBe(true);
    expect(result.unverified).toEqual([]);
  });

  it('the directive tells the verifier it is looking at a window, in both coordinate systems, and that missing text is not contradiction', async () => {
    const ledger = makeLedger();
    const source = registerDiff(ledger, 'src/big.ts', bigPatch(120), 120);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-window' }, source, 'src/big.ts', 118, 119));
    const seam = recordingSeam();

    await runContradictionChecks([finding], context(seam, ledger));

    const directive = seam.directives[0] as string;
    // Patch lines (what it was handed) and file lines (what the claim names) are stated separately.
    expect(directive).toMatch(/^window: lines \d+-\d+ of the 121 lines of this source's exact bytes, containing the cited file lines src\/big\.ts:118-119, new side\.$/m);
    expect(directive).toContain('a window of the exact bytes');
    expect(directive).toContain('text missing from this window is not evidence against the claim');
  });

  it('never sends a directive over the per-turn prompt cap — the cap bounds the whole prompt, the excerpt budget bounds the evidence, and the smaller one wins', async () => {
    // The adversarial finding this closes: this pass assembles its own prompt text and hands it
    // straight to `askModel`, so `renderModelPrompt`'s ceiling never applied to it. A directive
    // was measured at multiples of the configured cap and sent anyway, with the breach logged.
    const ledger = makeLedger();
    const source = registerDiff(ledger, 'src/big.ts', bigPatch(120), 120);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-capped' }, source, 'src/big.ts', 118, 119));
    const seam = recordingSeam();

    const CAP = 3_000;
    const result = await runContradictionChecks([finding], context(seam, ledger, { policy: testPolicy({ maxPromptBytesPerTurn: CAP }) }));

    const directive = seam.directives[0] as string;
    expect(Buffer.byteLength(directive, 'utf8'), 'the directive is a model-facing prompt and the cap is a promise about every one of them').toBeLessThanOrEqual(CAP);
    // Shrinking the window is not allowed to shrink away the thing under judgement.
    expect(directive).toContain("const value118 = 'marker-118-");
    expect(directive).toContain("const value119 = 'marker-119-");
    expect(result.complete).toBe(true);
    expect(result.unverified).toEqual([]);
  ...
  it('lets the excerpt budget govern when the cap is generous, so a roomy review is unchanged', async () => {
    // The other half of "who bounds what": at the shipped 192 KB cap the excerpt is decided by the
    // 4,000-character quality bound alone, exactly as it was before the cap entered this pass.
    const ledger = makeLedger();
    const source = registerDiff(ledger, 'src/big.ts', bigPatch(400), 400);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-roomy' }, source, 'src/big.ts', 300, 301));
    const seam = recordingSeam();

    await runContradictionChecks([finding], context(seam, ledger, { policy: testPolicy({ maxPromptBytesPerTurn: 192 * 1024 }) }));

    const directive = seam.directives[0] as string;
    const quoted = directive.split('"""')[1] as string;
    expect(quoted.length, 'the excerpt budget, not the cap, is what bit here').toBeLessThanOrEqual(4_000);
    expect(quoted.length).toBeGreaterThan(3_000);
    expect(directiveFor(finding, source), 'and it is byte-for-byte the directive the unbounded builder produces').toBe(directive);
  });

  it('skips the check rather than sending a truncated one when no excerpt containing the citation fits the cap', async () => {
    const ledger = makeLedger();
    const source = registerDiff(ledger, 'src/big.ts', bigPatch(120), 120);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-nofit' }, source, 'src/big.ts', 118, 119));
    const seam = recordingSeam();

    const result = await runContradictionChecks([finding], context(seam, ledger, { policy: testPolicy({ maxPromptBytesPerTurn: 700 }) }));

    expect(seam.calls, 'no model call at all: the question would have had to be asked about the wrong bytes').toBe(0);
    expect(result.findings.map((f) => f.candidateId), 'and the finding survives — an unasked question is not a refutation').toEqual(['cand-nofit']);
    expect(result.complete).toBe(false);
    expect(result.unverified[0]?.candidateId).toBe('cand-nofit');
    expect(result.unverified[0]?.reason).toContain('Contradiction check not performed');
  });

  it('a page under the budget is sent whole, in the original wording, byte for byte', async () => {
    const ledger = makeLedger();
    const patch = '@@ -1,2 +1,2 @@\n+const a = 1;\n+const b = 2;';
    const source = registerDiff(ledger, 'src/small.ts', patch, 2);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-small' }, source, 'src/small.ts', 1, 2));
    const seam = recordingSeam();

    await runContradictionChecks([finding], context(seam, ledger));

    const directive = seam.directives[0] as string;
    expect(directive).toContain('Cited evidence (the exact bytes already returned to you for this source; do not re-read the file):');
    expect(directive).toContain(`"""\n${patch}\n"""`);
    expect(directive).not.toContain('window: lines');
  });

  it('a finding citing the START of an over-budget page still receives the cited lines', async () => {
    const ledger = makeLedger();
    const source = registerDiff(ledger, 'src/big.ts', bigPatch(120), 120);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-start' }, source, 'src/big.ts', 1, 2));
    const seam = recordingSeam();

    await runContradictionChecks([finding], context(seam, ledger));

    const directive = seam.directives[0] as string;
    expect(directive).toContain("const value1 = 'marker-1-");
    expect(directive).toContain("const value2 = 'marker-2-");
    // The window opens at the top of the payload, so the hunk header comes with it.
    expect(directive).toContain('window: lines 1-');
  ...
  it('a cited range larger than the budget is not truncated: no model call at all, the finding kept, the stage incomplete', async () => {
    ...
    expect(result.findings.map((f) => f.candidateId)).toEqual(['cand-huge']);
    expect(result.contradicted).toEqual([]);
    expect(result.complete).toBe(false);
    expect(result.unverified).toHaveLength(1);
    expect(result.unverified[0]?.candidateId).toBe('cand-huge');
    expect(result.unverified[0]?.reason).toContain('Contradiction check not performed');
  ...
  it('skipping an unshowable excerpt does not spend the shared repair allowance the next finding needs', async () => {
    ...
    const result = await runContradictionChecks([unshowable, malformed], context(seam, ledger, { policy: testPolicy({ protocolRepairsPerPhase: 2 }) }));

    // One first ask plus the full allowance of two repairs, all spent on the second finding.
    expect(seam.calls).toBe(3);
    expect(result.unverified.map((entry) => entry.candidateId)).toEqual(['cand-a-huge', 'cand-b-garbage']);
    expect(result.findings).toHaveLength(2);
    expect(result.complete).toBe(false);
  });

  it('a citation that cannot be located in the payload leaves the check unperformed rather than guessing at a window', async () => {
    const ledger = makeLedger();
    // Positions say lines 1-3 are citable; the payload is one long line with no hunk structure at all.
    const source = registerDiff(ledger, 'src/opaque.ts', 'z'.repeat(5_000), 3);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-opaque' }, source, 'src/opaque.ts', 2, 3));
    const seam = recordingSeam();
    ...
    expect(result.complete).toBe(false);
    expect(result.unverified[0]?.reason).toContain('could not be located');
  });

  it('a line-aligned payload windows by arithmetic on its own range, not by hunk parsing', () => {
    const ledger = makeLedger();
    const anchorSource = registerDiff(ledger, 'src/big.ts', bigPatch(120), 120);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-file' }, anchorSource, 'src/big.ts', 1, 1));
    // A file read covering lines 500-700 of the file: payload line 1 is file line 500.
    const text = Array.from({ length: 201 }, (_, i) => `const fileLine${500 + i} = '${'y'.repeat(40)}';`).join('\n');
    const outcome = ledger.registerFileRange('m1', { state: 'complete', snapshot: SNAP, value: { path: 'src/read.ts', revision: 'head', startLine: 500, endLine: 700, text } });
    if (!outcome.ok) throw new Error(outcome.message);
    const fileSource = outcome.source;
    const cited: ValidatedFinding = {
      ...finding,
      evidence: {
        ...finding.evidence,
        primary: { ...finding.evidence.primary, sourceId: fileSource.sourceId, digest: fileSource.digest, path: 'src/read.ts', range: { startLine: 690, endLine: 691 } },
      },
    };

    const excerpt = selectEvidenceExcerpt(cited, fileSource);

    expect(text.length).toBeGreaterThan(4_000);
    expect(excerpt.kind).toBe('window');
    if (excerpt.kind !== 'window') throw new Error('expected a window');
    expect(excerpt.text).toContain('const fileLine690 =');
    expect(excerpt.text).toContain('const fileLine691 =');
    expect(excerpt.text).not.toContain('const fileLine500 =');
    // File line 690 is payload line 191 of 201.
    expect(excerpt.firstLine).toBeLessThanOrEqual(191);
    expect(excerpt.lastLine).toBeGreaterThanOrEqual(192);
    expect(excerpt.totalLines).toBe(201);
  });

  it('a verdict of "contradicted" on evidence the model was actually shown still removes the finding', async () => {
    const ledger = makeLedger();
    const source = registerDiff(ledger, 'src/big.ts', bigPatch(120), 120);
    const finding = accept(ledger, candidateRaw({ candidateId: 'cand-still-removed' }, source, 'src/big.ts', 118, 119));
    const seam = recordingSeam(true);

    const result = await runContradictionChecks([finding], context(seam, ledger));

    expect(seam.directives[0]).toContain("const value118 = 'marker-118-");
    expect(result.findings).toHaveLength(0);
    expect(result.contradicted).toEqual([{ candidateId: 'cand-still-removed', reason: 'The cited lines do the opposite of the claim.' }]);
    expect(result.unverified).toEqual([]);
    expect(result.complete).toBe(true);
  });
});
...
describe('runContradictionChecks (task 10.6, stage 2)', () => {
  ...
  it('zero findings makes zero model calls and reports complete', async () => {
    ...
    expect(result).toEqual({ findings: [], contradicted: [], unverified: [], complete: true });
    expect(seam.calls).toBe(0);
  });
  ...
  it('cancellation stops the stage and keeps every not-yet-checked finding unconfirmed rather than dropping it', async () => {
    ...
    // Cancellation gets no per-finding records: the attempt records its own cancellation, and one
    // entry per remaining finding would say nothing the cancelled run does not already say.
    expect(result.unverified).toEqual([]);
  });
...
describe('createSynthesisVerification (the injected SynthesisVerificationRunner)', () => {
  ...
  it('a finding whose cited evidence cannot be shown honestly survives, is reported in output.unverified, and fails the pass flags', async () => {
    const ledger = makeLedger();
    ...
    expect(output.findings.map((f) => f.candidateId)).toEqual(['cand-p4']);
    expect(output.contradicted).toEqual([]);
    expect(output.unverified?.[0]?.candidateId).toBe('cand-p4');
    expect(output.deduplicationComplete).toBe(true);
