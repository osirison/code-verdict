 ...
 * changed file's own directory, reading `AGENTS.md` at each level at the base
 * SHA.
 *
 ...
 * `AGENTS.md` is a file in the repository at a pinned commit, so it is exactly
 * what the rule assigns to git: everything answerable from two commits is
 * answered locally, and the provider is asked only for what is not in the
 * repository. Same neutral operation, same three-valued outcome; what changed
 * is who answers it.
 *
...
async function fetchLevel(
  ...
  if (!source) {
    return { directory, state: 'unavailable', reason: 'This review has no source that can read repository files.' };
  }
  if (capabilities && !capabilities.fileReads.supported) {
    return { directory, state: 'unavailable', reason: 'This source does not support pinned file reads.' };
  }
...
export function createAgentsPolicyResolver(
  ...
  function cachedLevel(member: AgentsPolicyMemberRef, directory: string): Promise<AgentsPolicyLevel> {
    ...
    const promise = fetchLevel(getSource(member), options.capabilities?.(member), member, directory, maxLinesPerFile);
