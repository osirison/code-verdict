...
import {
  ...
  type BaseRevisionKind,
  type InvestigationSourceKind,
  type ReviewRunAttachmentSnapshot,
  type ReviewRunMemberSnapshot,
...
export const RESUME_INCOMPATIBILITY_CODES = [
  ...
  // `add-local-git-investigation` task 9.5. Two dimensions, not one, because
  // they fail for different reasons and a reviewer can act on only one of them:
  // the source that produced the evidence, and what the recorded base commit
  // was taken to mean.
  'investigationSource',
  'baseRevisionMeaning',
] as const;
...
/**
 * Plain words for the two fields task 9.5 compares, written once so a reason
 * cannot name the same thing two ways. Neither carries an identifier a reviewer
 * would have to look up: what matters to them is that the answer came from
 * somewhere else, not what the host calls it.
 */
const SOURCE_LABELS: Record<InvestigationSourceKind, string> = {
  localGit: 'a local copy of the repository',
  sample: 'built-in sample data',
  // Nothing writes this any more — no source asks a platform for a diff. It is
  // here because checkpoints written before that change carry it, and the whole
  // point of comparing the kind is to tell a reviewer, in words, that a resumed
  // attempt would be reading from somewhere else than the stored one did.
  provider: 'the platform this change request lives on',
};

const BASE_REVISION_LABELS: Record<BaseRevisionKind, string> = {
  mergeBase: 'the commit this change branched from',
  targetBranchTip: 'the tip of the target branch',
};
...
function checkMemberDimensions(stored: ReviewRunMemberSnapshot, candidate: ReviewRunMemberSnapshot, reasons: Limitation[]): void {
  ...
  // `add-local-git-investigation` task 9.5, and the order is the point: what
  // the recorded commit *means* is compared before the commit itself, because
  // comparing two 40-hex strings first answers the wrong question when the
  // meaning behind them changed. Nothing in a sha says whether it is a merge
  // base or the tip of a target branch.
  if (investigationSourceKindOf(stored) !== investigationSourceKindOf(candidate)) {
    reasons.push(
      reason(
        'investigationSource',
        `Member ${stored.memberId}'s investigation was served by ${SOURCE_LABELS[investigationSourceKindOf(stored)]} and would now be served by ${SOURCE_LABELS[investigationSourceKindOf(candidate)]}. The two do not produce interchangeable evidence: the same file's diff has different bytes and the same change has a different list of files, so a new attempt starts over rather than mixing them.`,
      ),
    );
  }
  const storedBaseKind = baseRevisionKindOf(stored);
  const candidateBaseKind = baseRevisionKindOf(candidate);
  if (storedBaseKind !== candidateBaseKind) {
    // Design D4's two branches, and only one of them is a reason. When the
    // commit is byte-identical, the pair of commits the stored evidence was
    // computed over is the pair this attempt would compute over — so the
    // meaning alone settles nothing and the new attempt simply records the
    // current one (`buildReviewRunSnapshot` writes `mergeBase` for every
    // snapshot it builds).
    if (stored.baseSha !== candidate.baseSha) {
      reasons.push(
        reason(
          'baseRevisionMeaning',
          `Member ${stored.memberId}'s base revision was recorded as ${BASE_REVISION_LABELS[storedBaseKind]} and this attempt's is ${BASE_REVISION_LABELS[candidateBaseKind]}, so the stored commit was written under the earlier meaning and the two describe different commits.`,
        ),
      );
    }
  } else if (stored.baseSha !== candidate.baseSha) {
    // The same meaning, a different commit — the ordinary "the base moved"
    // fact, and the only case this wording is true for. Reporting it for a
    // changed *meaning* would tell a reviewer their target branch moved when
