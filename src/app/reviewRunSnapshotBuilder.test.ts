...
import { baseRevisionKindOf, investigationSourceKindOf } from '../domain/reviewRunSnapshot';
import {
...
import type { ProviderCapabilities, MemberCapabilities } from '../platform/provider';
...
describe('buildReviewRunSnapshot (task 6.1)', () => {
  ...
  /**
   * Task 5.4. Written on every member, not carried from the input: the
   * neutral contract specifies the merge base for every provider now, and a
   * provider that cannot determine one fails instead of substituting the
   * target-branch tip — so a snapshot built today cannot be a record of
   * anything else, and saying so explicitly is what lets a record written
   * before the change be read as the tip it actually holds.
   */
  it('records every member\'s base revision as the merge base', () => {
    const snapshot = buildReviewRunSnapshot(
      baseInput({
        targetKind: 'changeset',
        changesetId: 'cs-1',
        members: [member(), member({ memberId: 'm2', ref: { repoId: 'repo-2', number: '77' } })],
      }),
    );
    expect(snapshot.members.map((m) => m.baseRevisionKind)).toEqual(['mergeBase', 'mergeBase']);
    expect(snapshot.members.every((m) => baseRevisionKindOf(m) === 'mergeBase')).toBe(true);
  });

  /**
   * Task 9.4. The selected source is recorded beside `providerCapabilitySignature`,
   * never instead of it: the provider still serves change-request details, the
   * head check and posting, so both statements are true at once and each
   * describes a different answerer.
   */
  it('records the investigation source the caller selected, and writes no key at all when it selected none', () => {
    const selected = { kind: 'localGit' as const, contractVersion: '1', capabilitySignature: 'sig-local-source' };
    const withSource = buildReviewRunSnapshot(baseInput({ members: [member({ investigationSource: selected })] }));
    expect(withSource.members[0]!.investigationSource).toEqual(selected);
    expect(typeof withSource.members[0]!.providerCapabilitySignature).toBe('string');
    expect(investigationSourceKindOf(withSource.members[0]!)).toBe('localGit');

    // Absent, not present-and-undefined: absence is what a stored record that
    // predates the field looks like, and it has to keep meaning the provider.
    const withoutSource = buildReviewRunSnapshot(baseInput({ members: [member()] }));
    expect('investigationSource' in withoutSource.members[0]!).toBe(false);
    expect(investigationSourceKindOf(withoutSource.members[0]!)).toBe('provider');
  });
