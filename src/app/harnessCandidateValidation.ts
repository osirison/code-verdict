...
export function parseCandidateFinding(raw: unknown): { candidate: CandidateFinding } | { reasons: ValidationReason[] } {
  ...
  if (typeof raw.severity !== 'string' || !SEVERITIES.has(raw.severity)) {
    reasons.push({ code: 'schema', message: `severity must be one of ${SEVERITY_ORDER.map((s) => `"${s}"`).join(', ')}.` });
  }
  if (typeof raw.category !== 'string' || !CATEGORIES.has(raw.category)) {
    reasons.push({ code: 'schema', message: `category must be one of ${ALL_CATEGORIES.map((c) => `"${c}"`).join(', ')}.` });
  }
...
export function validateCandidate(raw: unknown, context: CandidateValidationContext): CandidateValidationOutcome {
  ...
  if (resolved.primary.ok) {
    ...
    // file failed a naive test that a quote spanning two hunks would have passed. The rule and the
    // reason it is exact rather than loose live in `./harnessCitations`.
    if (candidate.code !== undefined && candidate.code !== '' && !quotedTextAppearsIn(primary.source, candidate.code)) {
      rejected.push({
        code: 'codeNotInEvidence',
        message:
          primary.source.kind === 'diff'
            ? 'The quoted code is not consecutive lines of one side of one hunk of the primary evidence. Quote the lines as the file reads, without the leading "+", "-" or space of the patch.'
            : 'The quoted code does not appear in the primary evidence returned to the model.',
      });
    }
