...
import type {
  ...
  ReviewRunInvestigationSource,
  ReviewRunMemberSnapshot,
  ReviewRunSnapshot,
  ReviewRunTargetKind,
} from '../domain/reviewRunSnapshot';
import type { ChangeRequestRef } from '../platform/types';
import type { MemberCapabilities, ProviderCapabilities } from '../platform/provider';
...
export interface ReviewRunSnapshotMemberInput {
  ...
  /** The merge base, computed locally by the caller before this runs — never a value a platform reported. */
  baseSha: string;
  headSha: string;
  capabilities: MemberCapabilities;
  /**
   * Which source this member's investigation was selected to use, decided by
   * the caller before this runs (`investigationSourceSelection.ts`, task 9.1) —
   * like `baseSha` and `rootAgentsPolicy`, already resolved, never looked up
   * here. Optional only so a caller that does not select one (the harness's own
   * unit tests, which construct snapshots directly) still builds a snapshot;
   * every production path supplies it, and absence on a *stored* record means
   * the provider (`investigationSourceKindOf`).
   */
...
function buildMemberSnapshot(member: ReviewRunSnapshotMemberInput): ReviewRunMemberSnapshot {
  return {
    ...
    // Written unconditionally, and not taken from the caller: `baseSha` reaching
    // this builder is the merge base `git merge-base` computed during source
    // selection, over the pinned head and the branch the change request targets
    // (`investigationSourceSelection.ts`). There is no live path that can
    // produce a snapshot of anything else — a member whose merge base could not
    // be computed does not get a source and its attempt refuses — so an input
    // field for the other value would only invite recording a meaning nothing
    // produces. What the field is for is the records that predate it, which
    // were written from the target-branch tip; see `baseRevisionKindOf`.
    baseRevisionKind: 'mergeBase',
    headSha: member.headSha,
    // Written only when the caller selected one, so the field a stored record
    // does not carry keeps meaning what `investigationSourceKindOf` says it
    // means — the provider — rather than being written as an explicit
    // `undefined` that a reader then has to treat as a third case.
    ...(member.investigationSource ? { investigationSource: member.investigationSource } : {}),
