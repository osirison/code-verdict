...
function fullCapabilities(): MemberCapabilities {
  return {
    ...
    detailRetrieval: { changeRequestDetails: { supported: true, pageBound: { maxPageSize: 100 } }, issueDetails: { supported: true, pageBound: { maxPageSize: 100 } }, pagination: { maxPageSize: 100 } },
    reviewInvestigation: {
...
interface FakeConnectionOptions {
  ...
  readonly readFile?: InvestigationOperations['readFile'];
  readonly listChangedFiles?: InvestigationOperations['listChangedFiles'];
...
function memberInput(memberId: string, connection: Connection & Partial<InvestigationOperations>, attachments?: readonly Attachment[]): HarnessAttemptMemberInput {
  return { memberId, connection, capabilities: fullCapabilities(), investigationSource: fakeInvestigationSource(connection), attachments };
}
