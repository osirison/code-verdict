...
import { createDemoInvestigationSource } from '../registry';
import { getProvider } from '../platform/registry';
import type { InvestigationSource } from '../platform/types';
...
async function start(fixture: { repoId: string; number: string; baseSha: string; headSha: string }, policy = POLICY): Promise<Harness> {
  // The five pinned operations come off the demo pod's sample source now, not
  // off its connection — no provider answers one.
  const conn = createDemoInvestigationSource();
  ...
  // The head check stays a question for the forge — a fact about where a branch
  // points now, which no object store can answer — so it still goes through a
  // connection while every read above goes through the source.
  const head = await getProvider('fixture')
    .connect({ instanceUrl: 'fixture', credential: { kind: 'token', token: 'demo' } })
    .getCurrentHead?.({ repoId: fixture.repoId, number: fixture.number });
