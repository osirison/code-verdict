...
/**
 * Sums one optional numeric field across a folded run, `undefined` in `undefined` out (never a
 * fabricated 0): if no event in the run carries the field, the fold reports it as unknown, exactly
 * as a single unfolded event would.
 */
function sumOptional(run: readonly ToolCompletedEvent[], field: 'durationMs' | 'bytesSent' | 'bytesReceived' | 'retryWaitMs' | 'retryCount'): number | undefined {
  let total = 0;
  let any = false;
...
/**
 * Folds a run of 2+ same-tool `toolCompleted` events into one. D13's own rule is "coalesce while
 * keeping aggregate counts" — timing/size metadata (added for the run diagnostics time breakdown)
 * is exactly such an aggregate count, so every numeric field sums across the run rather than
 * silently keeping only the last event's value; a fold that dropped four calls' worth of duration
 * down to one would make the persisted, rebuilt-from-disk report undercount exactly where a
 * reviewer needs it least. `memberId`/`resultState` (identity-shaped, not additive) keep the last
 * event's value, matching how `tool` itself already does.
 */
function foldToolCompletedRun(run: readonly ToolCompletedEvent[]): ActivityEvent {
  ...
  const durationMs = sumOptional(run, 'durationMs');
  const bytesSent = sumOptional(run, 'bytesSent');
  const bytesReceived = sumOptional(run, 'bytesReceived');
  const retryWaitMs = sumOptional(run, 'retryWaitMs');
  const retryCount = sumOptional(run, 'retryCount');
  return {
    ...last,
    summary,
    ...
    ...(retryWaitMs !== undefined ? { retryWaitMs } : {}),
