...
function mutCapabilities(): MemberCapabilities {
...
describe('16.7: inventoryCompleteForEveryMember is load-bearing — a real truncated manifest blocks completion, alone', () => {
  /**
   * Truncated from the attempt's own first enumeration, which is also the run's
   * first.
   *
   * It briefly had to enumerate completely once and truncate on the next call:
   * source selection ran a serviceability check that consumed a run's first
   * `listChangedFiles` and refused a source whose manifest was not `complete`,
   * so a manifest truncating from the start ended the attempt before it reached
   * this clause. That check is gone, and the straightforward case is testable
   * again.
   */
...
function fullCapabilities2(): MemberCapabilities {
  return {
    suggestions: false,
    approvals: false,
    requestChanges: false,
    ...
    // pinned operations below are the member's composed set, which
    // `withSourceInvestigation` builds from a source — no provider declares one.
    detailRetrieval: { changeRequestDetails: PAGE_BOUND, issueDetails: PAGE_BOUND, pagination: { maxPageSize: 100 } },
    reviewInvestigation: {
...
function member2(connection: Connection & Partial<InvestigationOperations>): HarnessAttemptMemberInput {
  return { memberId: MEMBER2_ID, connection, capabilities: fullCapabilities2(), investigationSource: fakeInvestigationSource(connection) };
}
