...
function parseRetainedEvidenceRecord(raw: unknown): RetainedEvidenceRecord | undefined {
  ...
  // Absent is the pre-task-10.1 record, which `evidenceProducerOf` reads as the
  // provider — the only source that existed when it was written. Present but
  // not one of the three known producers is a malformed record, dropped whole
  // like every other unparseable field here.
  if (raw.producedBy !== undefined && !(EVIDENCE_PRODUCERS as readonly unknown[]).includes(raw.producedBy)) return undefined;
  return {
    ...
    ...(raw.producedBy !== undefined ? { producedBy: raw.producedBy as EvidenceProducer } : {}),
  };
}
...
function parseChangedFileRecord(raw: unknown): ChangedFileRecord | undefined {
  ...
  // Absent is a record written before declined content existed, and no such
  // record can have been declined; present-but-not-a-boolean is malformed and
  // fails the whole record closed like every other field here.
  if (raw.contentDeclined !== undefined && typeof raw.contentDeclined !== 'boolean') return undefined;
  // Same reading for the same reason: absent is a record written before a
  // failed read was recorded at all, and no such record can have one.
  if (raw.readFailed !== undefined && typeof raw.readFailed !== 'boolean') return undefined;
  return {
    path: raw.path,
    memberId: raw.memberId,
    state,
    ...
    ...(raw.contentDeclined === true ? { contentDeclined: true } : {}),
    ...(raw.readFailed === true ? { readFailed: true } : {}),
  };
}
