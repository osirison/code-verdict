...
describe('binding (task 7.2)', () => {
  ...
  // The shape the only production source actually returns: `localGitSource.readDiff` sets
  // `positions: []` on every page. Taking locations from that field alone left every real diff
  // page with none, so every finding citing one was rejected `pathMismatch` for the very file it
  // had just read. Every other test here supplies positions no source produces.
  it('derives citable spans from the patch itself when a source names no positions', () => {
    const source = expectOk(ledger().registerDiffPage('m1', diffPage({ value: { path: 'src/auth/token.ts', patch: PATCH, positions: [] } })));
    expect(source.locations).toEqual([
      { path: 'src/auth/token.ts', range: { startLine: 1, endLine: 4 }, side: 'new' },
      { path: 'src/auth/token.ts', range: { startLine: 1, endLine: 2 }, side: 'old' },
    ]);
  });

  it('derives one span per hunk per side, never one span across the gap between hunks', () => {
    const twoHunks = '@@ -1,2 +1,2 @@\n line one\n-old two\n+new two\n@@ -90,1 +90,2 @@\n line ninety\n+added ninety-one';
    const source = expectOk(ledger().registerDiffPage('m1', diffPage({ value: { path: 'src/auth/token.ts', patch: twoHunks, positions: [] } })));
    expect(source.locations).toEqual([
      { path: 'src/auth/token.ts', range: { startLine: 1, endLine: 2 }, side: 'new' },
      { path: 'src/auth/token.ts', range: { startLine: 1, endLine: 2 }, side: 'old' },
      { path: 'src/auth/token.ts', range: { startLine: 90, endLine: 91 }, side: 'new' },
      { path: 'src/auth/token.ts', range: { startLine: 90, endLine: 90 }, side: 'old' },
    ]);
  });

  // The header says six lines; the page carries two. Believing the header would make four lines
  // the model never saw citable — the one guarantee this ledger exists to provide.
  it('derives spans from the lines a page really carries, not from the hunk header counts', () => {
    const cut = '@@ -1,6 +1,6 @@\n line one\n+added line two';
    const source = expectOk(ledger().registerDiffPage('m1', diffPage({ value: { path: 'src/auth/token.ts', patch: cut, positions: [] } })));
    expect(source.locations).toEqual([
      { path: 'src/auth/token.ts', range: { startLine: 1, endLine: 2 }, side: 'new' },
      { path: 'src/auth/token.ts', range: { startLine: 1, endLine: 1 }, side: 'old' },
    ]);
  });

  it('names the old side of a rename by its old path', () => {
    const renamed = diffPage({ value: { path: 'src/auth/token.ts', oldPath: 'src/auth/legacy.ts', isRenamed: true, patch: PATCH, positions: [] } });
    const source = expectOk(ledger().registerDiffPage('m1', renamed));
    expect(source.locations).toEqual([
      { path: 'src/auth/token.ts', range: { startLine: 1, endLine: 4 }, side: 'new' },
      { path: 'src/auth/legacy.ts', range: { startLine: 1, endLine: 2 }, side: 'old' },
    ]);
  });
  ...
  it('derives members from the immutable snapshot', () => {
    ...
    // Neither member records an investigation source, which is what every
    // snapshot written before task 9.4 looks like — read as the provider, the
    // only source that existed then (`investigationSourceKindOf`).
    expect(ledgerMembersFromSnapshot(snapshot)).toEqual([
      { memberId: 'm1', repositoryId: 'repo-1', baseSha: 'base-1', headSha: 'head-1', changeRequestNumber: '2841', investigationSourceKind: 'provider' },
      { memberId: 'm2', repositoryId: 'repo-2', baseSha: 'base-2', headSha: 'head-2', changeRequestNumber: '7', investigationSourceKind: 'provider' },
    ]);
  });
});
...
// ---- add-local-git-investigation, task 10.1: every record names its source ----------------

/**
 * Task 10.1: a coverage report, a limitation or a diagnostic must be able to
 * state where a run's evidence came from, which is only possible if the fact
 ...
 */
describe('the producing source is recorded on every evidence record (task 10.1)', () => {
  const LOCAL: EvidenceLedgerMember = { ...MEMBER, investigationSourceKind: 'localGit' };

  it('stamps investigation evidence with the member’s own source and everything else with who really served it', () => {
    const led = ledger({ members: [LOCAL] });
    const searchResult: RepositorySearchResult = {
      state: 'complete',
      snapshot: SNAPSHOT,
      value: [{ path: 'src/auth/token.ts', line: 3, excerpt: 'const token = 1;' }],
    };

    // The four the selected source answers: local git here, because that is
    // what this member's snapshot recorded.
    expect(expectOk(led.registerDiffPage('m1', diffPage())).producedBy).toBe('localGit');
    expect(expectOk(led.registerFileRange('m1', fileRange())).producedBy).toBe('localGit');
    expect(expectOk(led.registerRepositorySearch('m1', { snapshot: SNAPSHOT, revision: 'base', query: 'token' }, searchResult)).producedBy).toBe('localGit');

    // The forge's own half of the connection, which no object store can answer
    // and which stays with the provider whichever source was selected.
    expect(expectOk(led.registerChangeRequestDetail('m1', { state: 'complete', snapshot: SNAPSHOT, value: detail() })).producedBy).toBe('provider');
    expect(expectOk(led.registerIssueDetail('m1', { state: 'complete', snapshot: SNAPSHOT, value: detail() })).producedBy).toBe('provider');

    // The reviewer's own file and the host's derived intent: no investigation
    // source ever saw either.
    expect(expectOk(led.registerAttachment('m1', attachment(), sha256Hex(attachment().content))).producedBy).toBe('host');
    expect(expectOk(led.registerIntent('m1', 'title', 'Refresh token')).producedBy).toBe('host');
  });

  it('stamps the same four origins as provider evidence for a provider-served member', () => {
    const led = ledger();
    expect(expectOk(led.registerDiffPage('m1', diffPage())).producedBy).toBe('provider');
    expect(expectOk(led.registerFileRange('m1', fileRange())).producedBy).toBe('provider');
  });

  it('gives every origin exactly one producer, and never leaves one unaccounted for', () => {
    for (const origin of EVIDENCE_ORIGINS) {
      for (const kind of ['localGit', 'provider'] as const) {
        expect(EVIDENCE_PRODUCERS).toContain(evidenceProducerFor(origin, kind));
      }
    }
    // The whole point of the mapping: only the five pinned operations move with
    // the source. If a future origin is added to the investigation set without
    // being added here, this is what fails.
    const movesWithTheSource = EVIDENCE_ORIGINS.filter((origin) => evidenceProducerFor(origin, 'localGit') === 'localGit');
    // `agentsPolicy` joined them: `AGENTS.md` is a file in the repository at the
    // base revision, so the source that reads the change reads it too.
    expect([...movesWithTheSource].sort()).toEqual(['agentsPolicy', 'diffPage', 'diffSearch', 'fileRange', 'repositorySearch']);
  });

  it('carries the producer across persistence and back, and reads an unstamped record as the provider', () => {
    const original = expectOk(ledger({ members: [LOCAL] }).registerDiffPage('m1', diffPage()));
    const retained = toRetainedEvidenceRecord(original, { includeExactContent: true });
    expect(retained.producedBy).toBe('localGit');
    expect(evidenceProducerOf(retained)).toBe('localGit');

    // A record stored before this field existed. Absence is a known fact — the
    // provider was the only source there was — not an unknown to be ignored.
    expect(evidenceProducerOf({ ...retained, producedBy: undefined })).toBe('provider');
  });

  it('keeps the producer the fetching attempt recorded when a later attempt imports the record', () => {
    const first = ledger({ members: [LOCAL], attempt: 1 });
    const original = expectOk(first.registerDiffPage('m1', diffPage()));
    const retained = toRetainedEvidenceRecord(original, { includeExactContent: true });

    const resumed = ledger({ members: [LOCAL], attempt: 2 });
    const imported = expectOk(resumed.importRetainedSource(retained));
    expect(imported.producedBy).toBe('localGit');
    expect(imported.importedFromAttempt).toBe(1);
  });

  it('refuses to import a record into a member another source now serves', () => {
    // `harnessResume.ts` refuses a resume across a source-kind change outright
    // (task 9.8), so this is the second line of the same defence and can only
    // be reached by hand. It is worth having: an imported record keeps the
    // prior identifier, and one that landed here would be evidence attributed
    // to a source that never produced it.
    const original = expectOk(ledger({ members: [LOCAL] }).registerDiffPage('m1', diffPage()));
    const retained = toRetainedEvidenceRecord(original, { includeExactContent: true });
    const providerServed = ledger({ attempt: 2 });
    expect(providerServed.importRetainedSource(retained)).toMatchObject({ ok: false, code: 'snapshotMismatch' });
    expect(providerServed.size).toBe(0);
  });
});
