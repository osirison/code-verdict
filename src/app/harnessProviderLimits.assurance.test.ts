...
function limitCapabilities(): MemberCapabilities {
...
function baseDeps(connection: Connection & Partial<InvestigationOperations>): HarnessRuntimeDeps {
  return {
    // This suite drives the real runtime without a real object store, so it
    // supplies the member's source the way a host does. The five operations are
    // the same fake bag the connection carries; what changed is which object
    // answers them.
    investigationSource: () => fakeInvestigationSource(connection),
...
describe('16.3: an incomplete (truncated) manifest cannot yield a clean result', () => {
  /**
   * The manifest truncates on the attempt's own first enumeration, which is
   * also the run's first.
   *
   * It briefly had to truncate on the *second* call instead: source selection
   * ran a serviceability check that consumed the first `listChangedFiles` of a
   * run and refused a provider whose manifest was not `complete`, so a source
   * truncating from the start ended the attempt before the completion gate this
   * test is about. That check is gone with the provider investigation path, and
   * the straightforward case is testable again.
   */
...
describe('16.3: a manifest of unknown completeness cannot yield a clean result', () => {
  /**
   * `unknown` on the attempt's own enumeration, for the same reason the
   * truncated case above answers `complete` first: a manifest that comes back
   * Like the truncated case above, this is the run's own first enumeration
   * again: the serviceability check that used to consume it is gone.
   */
  it('a real listChangedFiles response of state "unknown" folds to enumeration unavailable and blocks completion', async () => {
    const connection = fakeConnection({
      getChangeRequestDetails: async () => detailResult(),
      listChangedFiles: async (request) => {
        return { snapshot: request.snapshot, state: 'unknown', reason: 'The source could not determine whether more files exist.' };
      },
      readDiff: async (request) => diffPage(request.path),
      getCurrentHead: async () => ({ repoId: REPO_ID, state: 'resolved', headSha: HEAD_SHA }),
    });
