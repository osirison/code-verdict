...
describe('path traversal (task 9.8 item 1)', () => {
  ...
  it('a NUL byte anywhere in the path is refused', async () => {
    const { dispatcher } = setup(makeMember(fakeConnection({ readDiff: notImplemented })));
    const result = await dispatcher.dispatch('investigating', readDiffRequest({ request: { path: 'src/foo' + String.fromCharCode(0) + '.ts' } }));
    expect(result).toMatchObject({ state: 'refused', code: 'invalidPath' });
  });

  it('a percent-encoded traversal or separator (%2e%2e, %2f, %5c) is refused', async () => {
    const { dispatcher } = setup(makeMember(fakeConnection({ readDiff: notImplemented })));
    const encoded = await dispatcher.dispatch('investigating', readDiffRequest({ request: { path: '..%2f..%2fetc%2fpasswd' } }));
    expect(encoded).toMatchObject({ state: 'refused', code: 'invalidPath' });
    const encodedDots = await dispatcher.dispatch('investigating', readDiffRequest({ request: { path: '%2e%2e/%2e%2e/etc/passwd' } }));
...
describe('cursor provenance (task 9.8 item 2)', () => {
  it('refuses a cursor this dispatcher never issued (forged)', async () => {
    const { dispatcher } = setup(makeMember(fakeConnection({ readDiff: notImplemented })));
    const result = await dispatcher.dispatch('investigating', readDiffRequest({ request: { path: 'src/foo.ts', cursor: 'never-issued' } }));
  ...
  it('refuses a mutated cursor (one character changed from an issued value)', async () => {
    ...
    const mutated = await dispatcher.dispatch('investigating', readDiffRequest({ request: { path: 'src/foo.ts', cursor: 'issued-cursor-1-x' } }));
  ...
  it('refuses the same cursor value replayed against a different scope (a different path)', async () => {
    ...
    const wrongScope = await dispatcher.dispatch('investigating', readDiffRequest({ request: { path: 'src/other.ts', cursor: 'shared-value' } }));
...
describe('stale revision (task 9.8 item 4)', () => {
  /**
   * This used to send a `snapshot` pinned to `a-different-head` and assert `revisionMismatch`. A
   * request can no longer carry a revision at all: `harnessToolDispatcher.ts`'s `snapshotOf`
   * builds the pin from the member, so there is nothing for an adversary — or a model that
   * mis-copied a 40-character sha, which is what this refusal spent its life catching — to set.
   *
   * What is adversarial about a revision now is a *result* claiming a different one, and that is
   * checked where it can be checked: `harnessInventory.ts` refuses a manifest page whose own
   * snapshot does not match the member (`snapshotMismatch`), which is the source's answer rather
   * than a copy of the question. This case asserts the request half is closed by construction.
   */
  it('sends the member\'s own commits to the source, leaving no revision for a caller to stale', async () => {
    let seen: unknown;
    const source = fakeInvestigationSource({
      readDiff: async (request) => {
        seen = request.snapshot;
        return { snapshot: request.snapshot, state: 'unavailable', reason: 'recorded' };
      },
    });
    const member: DispatcherMember = { ...makeMember(fakeConnection({ readDiff: notImplemented })), investigationSource: source };
    const { dispatcher } = setup(member);
    await dispatcher.dispatch('investigating', readDiffRequest({ request: { path: 'src/foo.ts' } }));
...
describe('unavailable capability (task 9.8 item 5)', () => {
  ...
  it('refuses when the method itself is absent, even though the capability is declared supported', async () => {
    // A *detail* read, because that is where the check still has something to
    // catch. The five pinned operations are required members of an
    // `InvestigationSource`, so a source that holds one cannot be missing one;
    // a `Connection` may legitimately define neither detail operation.
...
describe('page bounds (task 9.8 item 6)', () => {
  ...
  it('refuses a readFile line range wider than the policy page-size field', async () => {
    ...
    const result = await dispatcher.dispatch('investigating', {
      ...
      memberId: 'm1',
      elapsedMs: 0,
      request: { revision: 'base', path: 'src/foo.ts', startLine: 1, endLine: TEST_POLICY.diffOrFileReadPageLines + 50 },
    });
    expect(result).toMatchObject({ state: 'refused', code: 'outOfBounds' });
  });
});
