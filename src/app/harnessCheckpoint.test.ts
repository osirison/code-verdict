...
describe('buildCheckpoint (11.2): a fully populated checkpoint round-trips every field', () => {
  ...
  it('carries a toolCompleted/toolFailed event\'s run-diagnostics timing and retry metadata through the real sanitizer and buildCheckpoint unchanged — the checkpoint round trip the time-breakdown report depends on', () => {
    const events = realEvents([
      { kind: 'toolCompleted', tool: 'modelTurn', summary: 'Model call: 500 byte(s) sent, 40 byte(s) received.', durationMs: 120, bytesSent: 500, bytesReceived: 40 },
      { kind: 'toolCompleted', tool: 'readDiff', target: 'file1.ts', summary: '1 unit(s) returned.', durationMs: 75, memberId: 'm1', bytesReceived: 2048, resultState: 'complete', retryWaitMs: 30, retryCount: 1 },
      { kind: 'toolFailed', tool: 'readFile', target: 'file2.ts', reason: 'The provider returned unavailable.', durationMs: 15, memberId: 'm1', resultState: 'unavailable' },
    ]);

    const checkpoint = buildCheckpoint(baseInput({ activityEvents: events }), DEFAULT_HARNESS_POLICY);

    expect(checkpoint.activity).toHaveLength(3);
    const [turn, read, failed] = checkpoint.activity;
    expect(turn).toMatchObject({ kind: 'toolCompleted', tool: 'modelTurn', durationMs: 120, bytesSent: 500, bytesReceived: 40 });
    expect(read).toMatchObject({ kind: 'toolCompleted', tool: 'readDiff', durationMs: 75, memberId: 'm1', bytesReceived: 2048, resultState: 'complete', retryWaitMs: 30, retryCount: 1 });
    expect(failed).toMatchObject({ kind: 'toolFailed', tool: 'readFile', durationMs: 15, memberId: 'm1', resultState: 'unavailable' });

    // Survives serialization too — the actual shape written to disk, not just the in-memory object.
    const reparsed = JSON.parse(JSON.stringify(checkpoint)) as typeof checkpoint;
    expect(reparsed.activity[1]).toMatchObject({ durationMs: 75, bytesReceived: 2048, retryWaitMs: 30, retryCount: 1 });
  });
