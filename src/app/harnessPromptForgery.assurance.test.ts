...
describe('16.1 structural: change-request title & body (bootstrap, inline/complete path — JSON-escaped)', () => {
  it('a hostile title/body cannot forge host instructions, a tool, a source identifier, or completion — the invariant region is byte-identical to the benign render', () => {
    ...
    const benignPrompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'planning', repairInstruction: undefined, toolResults: [], envelope: benignEnvelope });
    const hostilePrompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'planning', repairInstruction: undefined, toolResults: [], envelope: hostileEnvelope });
...
describe('16.1 structural: review discussion (bootstrap, inline/complete path, and the truncated/summarized path)', () => {
  ...
  it('once a section is too large and gets summarized, the hostile text does not merely get confined — it is not rendered at all (D4: a truthful bounded summary, counts only)', () => {
    ...
    const prompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'planning', repairInstruction: undefined, toolResults: [], envelope: hostileEnvelope });
...
describe('16.1 structural: repository file/diff content (tool results — the prompt\'s own last section)', () => {
  it('hostile file content returned by a real tool result is confined to that result\'s own body; only the host-generated header carries the real, single sourceId/digest', () => {
    ...
    const benignPrompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'investigating', repairInstruction: undefined, toolResults: benignResults, envelope: baseEnvelope });
    const hostilePrompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'investigating', repairInstruction: undefined, toolResults: hostileResults, envelope: baseEnvelope });
...
describe('16.1 structural: linked-issue content (the `getIssueDetails` tool result — issue text has no bootstrap path today)', () => {
  it('a hostile issue title/body, returned only through the getIssueDetails tool result, is confined the same way file content is', () => {
    ...
    const benignPrompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'investigating', repairInstruction: undefined, toolResults: [benignResult], envelope: baseEnvelope });
    const hostilePrompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'investigating', repairInstruction: undefined, toolResults: [hostileResult], envelope: baseEnvelope });
    assertConfinedToMiddle(benignPrompt, hostilePrompt, HOSTILE_MARKER, false);
...
describe('16.1 structural: AGENTS.md policy — never rendered as text to the model at all', () => {
  it('resolvePolicy\'s tool-result content carries only presence/sourceId/digest per level, never the policy text — a hostile AGENTS.md file cannot reach the rendered prompt through this tool, so it cannot forge anything in it', () => {
    ...
    const prompt = renderModelPrompt({ policy: DEFAULT_HARNESS_POLICY, investigation: [], submissions: [], phase: 'investigating', repairInstruction: undefined, toolResults: [hostilePolicyResult], envelope: baseEnvelope });
...
function forgeCapabilities(): MemberCapabilities {
...
describe('16.1 behavioral: AGENTS.md policy cited by its real (non-citable) sourceId is rejected end to end', () => {
  it('the policy tool result never carries content the model could quote, and even its honest sourceId is rejected as non-citable when a candidate cites it', async () => {
    ...
    // This test's whole point is that `resolvePolicy` *succeeds* (a real sourceId comes back) and
    // citing it is still rejected — `scopeInvestigationToChangedFiles: true` would refuse
    // `resolvePolicy` outright (it rides on `fileReads`) before that property is ever exercised.
    // The default is `false` now, so this line states the value it needs rather than relying on it,
    // which is what keeps the scenario meaning the same thing if the default moves again.
    const deps: HarnessRuntimeDeps = { ...baseDeps(), runTurn, policy: normalizeHarnessPolicy({ scopeInvestigationToChangedFiles: false }) };
    const factory = createReviewHarnessFactory(deps);
