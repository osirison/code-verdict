...
let extraInstructionsTimer;
document.addEventListener('input', (ev) => {
  if (ev.target.id !== 'extra') return;
  clearTimeout(extraInstructionsTimer);
  extraInstructionsTimer = setTimeout(() => post({ type: 'setInstructions', text: ev.target.value }), 250);
});
