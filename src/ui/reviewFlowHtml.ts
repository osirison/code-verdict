...
export interface FlowViewState {
  ...
  retainedDetails?: {
    ...
    /** The model's own closing statement — see `RetainedResult.conclusion`. Rendered as the
     * headline on a clean run, where it is the only thing on the page that says what was checked. */
    conclusion?: string;
  };
...
/* The model's closing statement. Deliberately louder than the lede beneath it: on a clean run this
   is the only sentence on the page that says what was actually checked, and burying it at the same
   weight as a file count is how it went unread in the first place. */
.conclusion {
  font-size: 13.5px;
  line-height: 1.65;
  color: var(--fg);
 ...
 */
/**
 * The model's own closing statement, given the most prominent place on the page after the headline.
 *
 * A clean run used to render a tick, "No findings above your criteria", a count of files read and
 * an Approve button — and nothing at all about what the model actually looked at. The sentence
 * that says why was parsed, sanitized, and dropped. A real review of a dependency bump concluded
 * "the direct Astro 7.2.10 version satisfies the updated Starlight peer requirement, and
 * package-lock resolutions are aligned", which is precisely the reasoning a reviewer needs in
 * order to trust an approval, and none of it reached the screen.
 *
 * Unlike the legacy notice below, this survives `protocolProvenance: 'legacy-one-shot'`: it is the
 * model's own words, not a plan/coverage/evidence report the harness never produced, so D16's
 * "must not invent detail for a review the harness never ran" does not apply to it.
 */
function conclusionBlock(s: FlowViewState): string {
  const conclusion = s.retainedDetails?.conclusion?.trim();
  if (!conclusion) return '';
  return `<p class="conclusion">${e(conclusion)}</p>`;
}

function retainedDetailsBlock(s: FlowViewState): string {
  const details = s.retainedDetails;
  if (!details) return '';
  // The clean screen renders the conclusion as its own headline, directly under the h1, so it must
  // not appear a second time here. Every other screen that shows retained detail gets it at the
  // top of the block — a review that *did* find something still benefits from the model's own
  // account of what it looked at, and that account is not a plan/coverage report D16 forbids
  // inventing, so it is emitted before the legacy check rather than after it.
  const conclusion = s.screen === 'clean' ? '' : conclusionBlock(s);
  if (details.protocolProvenance === 'legacy-one-shot') {
    return `${conclusion}<div class="legacy-notice">Legacy review — it ran before this change recorded plan, activity, and coverage detail. None of that detail is available for it.</div>`;
  }
