...
export class VerdictSidebarProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    this.ready = false;
    // Before the options, never after (see renderSidebarPlaceholderHtml):
    // VS Code answers a content-options change by re-sending the html the
    // view is currently holding, which is the empty string it was created
    // with until something assigns one — and an empty document has no CSP,
    // which is the warning a real session logged 450ms after this activation
    // event. Assigned here rather than through paint(), which stores the pod
    // data it is handed as the provider's held state: a placeholder has none
    // to hand it, and passing one would overwrite what is already held.
    view.webview.html = renderSidebarPlaceholderHtml(crypto.randomBytes(16).toString('hex'));
