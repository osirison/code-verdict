...
/**
 * The document the view is given *before* its options are, so that the first
 * thing VS Code parses for this webview already carries the CSP.
 *
 * The live failure (a real session, VS Code's own log):
 *
 ...
 *
 * Nothing this file renders was missing one. The document without a CSP was
 * the empty string the view is created with: `WebviewViewPane.activate()`
 * builds the webview with `contentOptions: {}` and `html: ""`, and VS Code
 * answers a later content-options change by re-sending the html it is
 * currently holding — so assigning `webview.options` first pushes `""` to
 * the webview, which parses it, finds no
 * `<meta http-equiv="Content-Security-Policy">` and reports `no-csp-found`.
 * Assigning this first means every content update that view ever receives
 * carries a policy.
 *
 * Deliberately script-free — `embedded` with no script and no `regions`
 * emits no bootstrap at all. The options assignment re-sends this same
 * document with scripts enabled, and the real paint can be a cold fetch
 * away; a placeholder that armed REGIONS_SCRIPT would post `verdictReady`,
 * mark the view ready, and let a `patch()` in that window post regions into
 * a document holding none of their ids, which is dropped silently. It
 * carries the theme's own CSS, so the moment before the first paint is the
 * editor's background rather than a white flash.
 */
export function renderSidebarPlaceholderHtml(nonce: string): string {
  return renderPage({ title: 'Verdict', nonce, css: '', body: '', embedded: true });
}
