...
export interface HarnessSettingsView {
  ...
  maxPromptKilobytesPerTurn: number;
...
export const HARNESS_NUMBER_FIELDS: ReadonlyArray<{
  ...
  {
    key: 'maxPromptKilobytesPerTurn',
    label: 'Prompt size per turn',
    hint: 'Maximum kilobytes in one prompt to the model — framing plus the diffs and results it carries. Lower it for a local model that stalls on large prompts.',
    min: 1,
  },
  {
...
function jsonPreviewRegion(state: SettingsViewState): string {
  const preview = JSON.stringify({
    ...
    'codeVerdict.harness.maxPromptKilobytesPerTurn': state.harness.maxPromptKilobytesPerTurn,
