...
export function renderChangesetHtml(state: ChangesetViewState, nonce: string): string {
  ...
  const script = `const vscode=acquireVsCodeApi();const post=(message)=>vscode.postMessage(message);document.querySelectorAll('[data-repo]').forEach((row)=>row.addEventListener('click',()=>post({type:'openMember',repoId:row.dataset.repo,number:row.dataset.number})));document.getElementById('review-together')?.addEventListener('click',()=>post({type:'reviewTogether',changesetId:'${e(state.id)}'}));document.getElementById('back')?.addEventListener('click',()=>post({type:'back'}));`;
  return renderPage({ title: `Verdict: Changeset · ${state.name}`, nonce, css: CSS, body, script });
}