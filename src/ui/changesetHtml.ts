...
export function renderChangesetHtml(state: ChangesetViewState, nonce: string): string {
  ...
  const script = `const vscode=window.verdictVscode;const post=(message)=>vscode.postMessage(message);document.querySelectorAll('[data-repo]').forEach((row)=>row.addEventListener('click',()=>post({type:'openMember',repoId:row.dataset.repo,number:row.dataset.number})));document.getElementById('review-together')?.addEventListener('click',()=>post({type:'reviewTogether',changesetId:${changesetId}}));document.getElementById('back')?.addEventListener('click',()=>post({type:'back'}));`;
