...
Investigation is split in two, and the split follows one rule: **anything answerable from two
commits is answered by git, against a local copy of the repository; the provider is asked only for
what is not in the repository files.**

`InvestigationSource` (`platform/types.ts`) is the five revision-pinned operations —
`listChangedFiles`, `readDiff`, `readFile`, `searchRepository`, `searchDiff` — and
`InvestigationSourceCapabilities` declares what a source supports, per operation, with its page-size
bound. There is one production implementation: the local git source reading a bare object store this
extension owns (`src/localgit/`). No provider implements any of the five, and
`providers/investigationBoundary.test.ts` fails if one grows one back.

That is a reversal. Every provider used to implement them, and a review fell back to the forge when
a local store could not be built. Measured against a 207-file, all-TypeScript change, GitHub's
compare response declined to render 137 of the diffs and reported them in the byte-identical shape a
binary file produces, so two thirds of the change was written off as uninspectable and the run still
called itself complete. The fix is not a better reading of that answer; it is not asking.

`ProviderCapabilities.detailRetrieval` is what is left on a provider: `changeRequestDetails` and
`issueDetails`, with their page bound. `Connection` keeps one optional method per surviving
operation (`getChangeRequestDetails`, `getIssueDetails`, `getCurrentHead`) plus `getObjectSource`,
which says where the objects can be fetched from, how to authenticate, and which branch the change
targets. Every request carries an explicit repository and base/head or single revision, and every
result echoes it back, so a caller can prove the answer is for the exact commit asked for rather
than a moving branch tip.

`withSourceInvestigation` (`app/harnessRuntime.ts`) composes the two declarations into the one set a
member's tools are gated on. The harness reads that declaration, never `ScmProvider.id`, so a gap
degrades a review honestly instead of substituting different data.

**The commit a diff is against is computed, not reported.** Acquisition fetches the pinned head and
the target branch, runs `git merge-base`, pins the result under its own ref, and hands it back for
the snapshot. A review that cannot reach git does not fall back — it ends before bootstrap with
completeness `none` and a reason naming what could not be obtained.

The computed answer is proved before it is used. `git merge-base` answers over the history that was
fetched and cannot say which history that was, so in a partial clone it can return an older common
ancestor than the real one — with nothing on the face of the answer to distinguish the two. A
candidate is accepted only when the fetched history stops nowhere between either pinned commit and
that candidate, which is what makes it the commit a full clone would report; otherwise acquisition
fetches deeper and asks again, and at its bound it refuses and names what it could not establish. It
never substitutes a commit it cannot stand behind, because reviewing the wrong comparison says
...
Budgets are per attempt and split into ordinary/high-risk/verification lanes before investigation
begins, so a large change can be arithmetically uncoverable at the defaults. Sizing the dials by
changed-file count is [`REVIEW-BUDGETS.md`](REVIEW-BUDGETS.md).
