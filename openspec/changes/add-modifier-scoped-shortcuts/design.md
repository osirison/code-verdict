...
One overlap survives and is out of scope: `ctrl+enter` exists at both layers, as `codeVerdict.askAgent` and as the `#ask` handler (`src/ui/reviewFlowHtml.ts:1747-1761`), and the two send *different* payloads — the command asks with the `explain` preset on the selected finding, the DOM handler asks freeform with the typed text. Whether that produces one ask or two today is unknown; task 6.7 checks it manually. It is left alone here because reconciling the two payloads is a change to what asking does, not to when a key fires. The overlay does name both results, though — an undocumented second behaviour is worse than a documented overlap.
