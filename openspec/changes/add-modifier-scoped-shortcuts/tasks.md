...
- [x] 5.5 Update `src/ui/theme.test.ts` (the overlay assertions at `:8-64`) and `src/ui/statusBar.test.ts:97`. Add a test that the overlay renders the `⌘` form under the macOS flag and the `⌃` form otherwise.
- [x] 5.6 Update `src/ui/reviewFlowHtml.test.ts:99-102` — it asserts the shortcut map's shape by string match and will still hold, but re-read it against the edited handler rather than assuming.
...
- [x] 6.1 `npm run typecheck && npm run lint && npm run test` all green.
...
- [x] 7.4 The rationale that has to survive in the repository is already in `design.md` — D4 for why every two- and three-key modifier family was unusable, D1 and D6 for what the two-layer split buys — and the operational preconditions in `docs/agent-notes/`. This task is the author's personal narrative record on top of those, kept outside the repo by their own convention; nothing a future contributor needs is only there.
