...
- [ ] 9.6 Move long retry delays to waiting with checkpoint and slot release, then through resuming with original queue fairness and one-active-run-per-target ownership preserved. (Policy-level classification, typed `wait` outcome, and `onCheckpointDue`/`onEnterWaiting`/`onResuming` hooks were already implemented and tested in `harnessRetry.ts`/`harnessRetry.test.ts`. Task 12.1 now closes the other half: `ReviewRunManager.enterPausedState`/`beginResuming` in `src/app/reviewRunManager.ts` release the concurrency slot immediately on entering `waiting`/`paused`, re-enter the FIFO queue on `resuming` sorted by each record's original `queuedAt` — never appended at the tail — and `active()`/`trigger()`'s admission check are keyed off `isTerminalLifecycle` so one-active-run-per-target holds through `waiting`/`paused`/`resuming` exactly as it does through `running`. Covered by new tests in `src/app/reviewRunManager.test.ts` ("task 12.4/9.6: waiting releases the slot..." and "task 12.2/12.4: one active run per target holds through waiting"). What remains open is *wiring* — a real harness attempt actually calling `RunnerOptions.onEnterWaiting`/`onResuming` in production — which is task 10.8/15.7's job, not this mechanism's.)
