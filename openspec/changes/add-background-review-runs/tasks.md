...
- [ ] 2.3 Replace the clean-run deletion (`src/ui/reviewFlow.ts:652`) with a `clean` record write.
- [ ] 2.3a Make the changeset clean branch (`src/ui/changesetReview.ts:337`) write a `clean` record too — today it persists nothing, so a reload after a clean re-run re-enters triage on the previous run's findings. Cover that case with a test.
...
- [ ] 2.5 Tests: a clean run leaves a re-openable record; a successful submit leaves a record with `submittedAt` and an empty ledger; a retry after a partial failure still refuses to re-post what landed.
...
- [ ] 5.1 Delete `runToken` and both bumps from `src/ui/reviewFlow.ts` (`:254`, `:317`); leave `loadSeq` alone. Do the same in `src/ui/changesetReview.ts` (`:145`, and the `cancel` case at `:393`).
- [ ] 5.2 Move `run()`, `finishRun()` and `recordRun()` out of both panels, demo branch included; `run()` becomes a trigger call carrying the snapshot, with the selection still persisted to the pod before triggering.
- [ ] 5.3 Subscribe each panel to the manager for its current target, and unsubscribe in `onLeave` without touching the record.
- [ ] 5.4 Rewrite `load()`'s routing to the four ordered arms in D9, including the retained-record table in D7b.
- [ ] 5.5 Add `{ type: 'newRun' }` to `FlowMessage` and the "Run a new review" control to the `triage`, `clean` and `done` screens; it opens the pickers pre-selected from the record and keeps a way back to the review (D7b). Leave the existing `rerun` message as it is.
- [ ] 5.6 Render the retained review beside a run in flight on the running screen, with a control back to it (D7c).
- [ ] 5.7 Confirm the staleness path still runs on open against a retained record (`isStale`, `markMoved`) — an old record must arrive labelled.
- [ ] 5.8 Tests in `src/ui/reviewFlowHtml.test.ts` and the panel tests: each of the three retained-record shapes renders its screen and carries the control; the running screen renders both; no new markup uses an inline `style=` attribute (webview CSP).

## 6. Concurrency setting

- [ ] 6.1 Add `codeVerdict.agentRun.maxConcurrent` to `package.json`, described with the `0` = no limit convention the two timeout settings already use.
- [ ] 6.2 Add `agentRunConcurrency()` to `src/ui/agentRunOptions.ts` with the same non-usable-value guard as `windowMs`, and test it (unset, `0`, negative, non-numeric, large).

## 7. Visibility outside the run screen

- [ ] 7.1 Sidebar: an active-runs section listing target, state, elapsed and a cancel control, fed by the manager's `onChange`.
- [ ] 7.2 Status bar: a running-review count segment, hidden at zero the way the bell already is.
- [ ] 7.3 Dashboard and changeset rows: a `Running…` / `Queued` pill from one extra map in `DashboardDeps`, keyed as `reviewRuns` already is.
- [ ] 7.4 Tests: the section and pills appear and clear with the manager's state; nothing is rendered at zero active runs.

## 8. Wiring, retention and verification

- [ ] 8.1 Construct the manager in `activate()`, wire `onRunRecorded`, `onReviewReady` and the `repaintReviewSurfaces` fan-out into it rather than through the panel deps, and run the sweep before the first render.
- [ ] 8.2 Cancel a pod's runs from the existing `deletePod` handler.
- [ ] 8.3 Drop retained records for change requests that are no longer open, on the poll that already lists them (`fetchPodData`).
- [ ] 8.4 Update `spec/specs/Code Verdict - naming & commands.md` with the cancel-run command and the new setting.
- [ ] 8.5 Run `npm run typecheck`, `npm run lint` and `npm test`, and report the output.
