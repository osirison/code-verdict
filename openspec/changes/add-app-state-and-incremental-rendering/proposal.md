...
**Navigation rebuilds the document.** There are nine webview surfaces: eight panel screens sharing one `WebviewPanel`, plus the sidebar, which is a separate `WebviewView`. The eight share that panel (`src/ui/appSurface.ts:104`, `retainContextWhenHidden: true`), so the panel is never destroyed — but every route entry still assigns a fresh `webview.html`. For the review flow that is a ~40 KB string (18.3 KB of CSS at `src/ui/reviewFlowHtml.ts:275`, 9.5 KB of script at `:1275`, plus ~12.6 KB of shared shell from `src/ui/theme.ts`) crossing the IPC boundary, reparsed into a new DOM, with every bootstrap script re-run and all scroll position, focus and expanded/collapsed state discarded. Issue #39 introduced the fix — `AppRoute.postRegions` patching named containers (`src/ui/appSurface.ts:71-76`, `src/ui/theme.ts:387-427`) — and it works, but only three of the nine use it. `changesetReview.ts`, `changeset.ts`, `settings.ts`, `tuning.ts`, `onboarding.ts` and the sidebar still full-replace on every state change, and no surface avoids the rebuild when it is the target of a navigation.
...
- A single **`AppStore`** owning the pod platform data every surface reads — open change requests, work items and CI runs — plus the changesets derived from it. Surfaces **subscribe** instead of being told to `refresh()` by a hand-maintained fan-out list. Run records stay with `ReviewRunManager` and connection status stays with the settings panel; the store does not absorb them.
...
- The **notifier's poll becomes the store's revalidation tick** instead of one more independent fetch of the same data. Its per-review thread fan-out stays where it is: threads are per change request, not pod-keyed.
...
- **In-progress typing survives a repaint.** Today it does not: the summary, note and instructions fields commit on `change` (blur), and the ask and reply inputs are never sent to the host at all, so any patch of the region containing them re-renders over what the reviewer was typing. Every editable's in-progress text moves into panel state on debounced `input`, which is what makes a re-render safe.
...
| `src/extension.ts` | Constructs the store once and injects it. `repaintReviewSurfaces` (`:165-170`) is replaced by store subscriptions. The `lastRunStatus` transition check (`:143-151`) stays where it is — the store holds pod data, not run records — but now gates a repaint from held state rather than a pod fetch. |
...
| `src/ui/settings.ts` | Connection status stays in the panel and is tested on open and on explicit re-test only; a config toggle patches its control instead of re-rendering and re-testing the connection. |
